...
(function ($) {
  ...
  function documentReadyInit() {
    ...
    //toggle mobile menu
    $(
      ".page_header .toggle_menu:not(.menu_widget), .page_toplogo .toggle_menu"
    ).on("click", function () {
      $(this)
    ...
    if ($().localScroll) {
      $(
        ".top-nav > ul, .mainmenu_side_wrapper > ul, #land, .comments-link, .scroll-link"
      ).localScroll({
        duration: 900,
      ...
      var ajax = jQuery
        ...
        .done(function (data) {
          $($form)
            ...
            .parent()
            .append(
              '<div class="contact-form-respond color-main2 mt-20">' +
                data +
                "</div>"
            );
          //cleaning form
        ...
        .fail(function (data) {
          $($form)
            ...
            .parent()
            .append(
              '<div class="contact-form-respond color-main2 mt-20">Mail cannot be sent. You need PHP server to send mail.</div>'
            );
        });
    });
  ...
  function windowLoadInit() {
    ...
    //flexslider-video
    const videoSliders = Array.from(
      document.querySelectorAll(".myVideo:not(canvas)")
    );
